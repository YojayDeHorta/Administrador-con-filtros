        ...
        <v-row class="Op-Editartext-center mt-5 d-flex">
            <div class=" ">
                <v-col class="Op-Iconos" >
                    <v-btn class="Btn-Icon" style="margin:auto" v-if="adminVerification" color="primary" @click="dialog = true; formTitle = 'Agregar usuario';
                  resetUser();
                ">
                        <v-icon class="mr-2">mdi-account-plus</v-icon> A
                    </v-btn>

                    <v-btn class="Btn-Icon" color="teal darken-1" @click="descargar()" v-if="adminVerification">
                        <v-icon style="margin:auto" color="white">mdi-cloud-download</v-icon>
                        &nbsp;&nbsp;Descargar Archivo
                    </v-btn>
                    <!--descarga normal-->
                    <v-btn  class="Btn-Icon"  color="teal darken-1 white--text" v-if="!adminVerification" @click="descargarFiltro()">
                        <v-icon color="white">mdi-file-download</v-icon>descargar
                        tabla
                    </v-btn>
                    <!--descarga filtrada-->
                    <v-btn color="secondary" class="Btn-Icon ml-1" outlined v-if="adminVerification" @click="chooseFiles()">
                        <span v-if="file == null">
